...
#' Generate Heatmaps of Top Factors Over Time (Backward Expanding Window)
#'
#' Reads results from backward time-varying estimation (.rds format) and generates
#' heatmaps showing the top factors by posterior probability over time with reversed x-axis.
#'
#' @param rds_path       Path to the ALL_RESULTS.rds file from run_time_varying_estimation()
#' @param psi_level      Shrinkage level to use: 0.2, 0.4, 0.6, or 0.8 (default: 0.8)
#' @param top_n          Number of top factors to show (default: 5)
#' @param output_path    Directory to save figures (default: "output/paper/figures")
#' @param figure_prefix  Prefix for output filenames (default: "fig6b")
#' @param verbose        Print progress messages (default: TRUE)
#'
#' @return List containing:
#'   - top_factors_prob: Named list of top factors by date
...
#'
#' @details
#' This function is similar to expanding_runs_plots() but uses a reversed x-axis
#' (scale_x_reverse) appropriate for backward-expanding window analysis where
#' the estimation windows expand backward in time.

expanding_runs_plots_reverse <- function(
    rds_path,
    psi_level     = 0.8,
    top_n         = 5,
    output_path   = "output/paper/figures",
  ...
  ## ── 4. Build long data frame for heatmap ──────────────────────────────────
  # Normalize dates to January 1st of each year
  df_long <- map_df(names(top_factors_prob), function(d) {
    yr <- format(as.Date(d), "%Y")
    tibble(
  ...
  df_long <- df_long %>%
    mutate(
      factor  = factor(factor, levels = rev(ordering)),
      DateNum = as.numeric(date)
    )

  unique_dates <- sort(unique(df_long$date))

  ## ── 6. Create heatmap with REVERSED x-axis ────────────────────────────────
  # Plotting parameters
  x_tick_size      <- 14
  legend_text_size <- 14
  y_label_size     <- 14
  ...
  # Create the plot with reversed x-axis
  p <- ggplot() +
    geom_tile(
      data   = df_long,
      aes(x = DateNum, y = factor, fill = Rank),
      colour = "white"
    ) +
    scale_x_reverse(
      breaks = as.numeric(unique_dates),
      labels = format(unique_dates, "%Y")
    ) +
    scale_fill_gradient(
      low = "#08519c",
      high = "#c6dbef",
...
#' Generate Figure 6 Panel B: Top Factors Heatmap (Backward Expanding)
#'
#' Convenience wrapper for expanding_runs_plots_reverse() with standard output naming.
#'
#' @param rds_path       Path to the backward ALL_RESULTS.rds file
#' @param psi_level      Shrinkage level (default: 0.8)
#' @param top_n          Number of top factors (default: 5)
#' @param output_path    Output directory
#' @param verbose        Print progress
...
#' Generate Heatmaps of Top Factors by Lambda (Backward Expanding Window)
#'
#' Reads results from backward time-varying estimation (.rds format) and generates
#' heatmaps showing the top factors by absolute market price of risk with reversed x-axis.
#'
#' @param rds_path       Path to the ALL_RESULTS.rds file from run_time_varying_estimation()
#' @param psi_level      Shrinkage level to use: 0.2, 0.4, 0.6, or 0.8 (default: 0.8)
#' @param top_n          Number of top factors to show (default: 5)
#' @param output_path    Directory to save figures (default: "output/paper/figures")
#' @param figure_prefix  Prefix for output filenames (default: "fig_ia_17b")
#' @param verbose        Print progress messages (default: TRUE)
#'
#' @return List containing:
#'   - top_factors_lambda: Named list of top factors by date
...
#'
#' @details
#' This function is similar to expanding_runs_plots_lambda() but uses a reversed x-axis
#' (scale_x_reverse) appropriate for backward-expanding window analysis.

expanding_runs_plots_lambda_reverse <- function(
    rds_path,
    psi_level     = 0.8,
    top_n         = 5,
    output_path   = "output/paper/figures",
...
#' Generate Figure IA.17b: Top Factors by Lambda Heatmap (Backward Expanding)
#'
#' Convenience wrapper for expanding_runs_plots_lambda_reverse() with standard output naming.
#'
#' @param rds_path       Path to the backward ALL_RESULTS.rds file
#' @param psi_level      Shrinkage level (default: 0.8)
#' @param top_n          Number of top factors (default: 5)
...
generate_figure_ia17b <- function(
    rds_path,
    psi_level   = 0.8,
    top_n       = 5,
    output_path = "output/paper/figures",
