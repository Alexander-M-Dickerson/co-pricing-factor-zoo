continuous_ss_sdf_fast_code_dir <- local({
  # Capture $ofile NOW during source(), not later at call-time.
  ...
  function() {
    ...
    stop("Could not determine code_base directory for continuous_ss_sdf_fast.")
  }
})

continuous_ss_sdf_fast_cpp_state <- local({
  state <- new.env(parent = emptyenv())
  state$attempted <- FALSE
  state$compiled <- FALSE
  state$last_error <- NULL
  state$cache_dir <- NULL
  state$cpp_name <- "continuous_ss_sdf_fast_cpp_impl"
  state$cpp_env <- new.env(parent = baseenv())
  state
})

continuous_ss_sdf_fast_cpp_cache_dir <- function() {
  state <- continuous_ss_sdf_fast_cpp_state
  if (!is.null(state$cache_dir) && dir.exists(state$cache_dir)) {
    return(state$cache_dir)
  }

  cache_dir <- file.path(
    dirname(continuous_ss_sdf_fast_code_dir()),
    ".cache",
...
load_continuous_ss_sdf_fast_cpp_from_cache <- function() {
  state <- continuous_ss_sdf_fast_cpp_state
  cache_dir <- continuous_ss_sdf_fast_cpp_cache_dir()
  wrapper_files <- list.files(
    cache_dir,
    pattern = "^continuous_ss_sdf_fast\\.cpp\\.R$",
    recursive = TRUE,
    full.names = TRUE
  )
...
prepare_continuous_ss_sdf_fast_cpp_inputs <- function(f,
                                                       R,
                                                       psi0 = 1,
                                                       r = 0.001,
                                                       intercept = TRUE) {
  ...
  BayesianFactorZoo:::check_input2(f, R)

  ones_N <- matrix(1, nrow = N, ncol = 1)
  if (intercept) {
    beta_ols <- cbind(ones_N, Corr_ols[(k + 1):p, 1:k, drop = FALSE])
  } else {
...
load_continuous_ss_sdf_fast_cpp <- function(force_rebuild = FALSE) {
  state <- continuous_ss_sdf_fast_cpp_state
  ...
  if (!force_rebuild && load_continuous_ss_sdf_fast_cpp_from_cache()) {
  ...
  cpp_file <- file.path(continuous_ss_sdf_fast_code_dir(), "continuous_ss_sdf_fast.cpp")
  ...
  compiled_ok <- tryCatch({
    Rcpp::sourceCpp(
      file = cpp_file,
      env = state$cpp_env,
      cacheDir = continuous_ss_sdf_fast_cpp_cache_dir(),
  ...
  if (!compiled_ok && is.null(state$last_error)) {
    state$last_error <- "C++ backend compiled without exporting continuous_ss_sdf_fast_cpp_impl."
  }

  compiled_ok
}

continuous_ss_sdf_fast_cpp_error <- function() {
  continuous_ss_sdf_fast_cpp_state$last_error
}

continuous_ss_sdf_fast_cpp_function <- function() {
  state <- continuous_ss_sdf_fast_cpp_state
  if (!exists(state$cpp_name, envir = state$cpp_env, inherits = FALSE)) {
    stop("continuous_ss_sdf_fast_cpp_impl is not loaded.", call. = FALSE)
  }
  get(state$cpp_name, envir = state$cpp_env, inherits = FALSE)
}

continuous_ss_sdf_fast_backend_status <- function() {
  state <- continuous_ss_sdf_fast_cpp_state
  list(
...
continuous_ss_sdf_fast_r <- function(f,
                                     R,
                                     sim_length,
                                     psi0 = 1,
                                     r = 0.001,
  ...
  sigma_shape <- if (intercept) (N + k + 1) / 2 else (N + k) / 2
  ...
  for (i in seq_len(sim_length)) {
    ...
    C_f <- corr_Y[(k + 1):p, 1:k, drop = FALSE]
    a <- mu[(1 + k):p, 1, drop = FALSE] / sd_Y[(1 + k):p]
    if (intercept) {
      beta <- cbind(ones_N, C_f)
    } else {
      beta <- matrix(C_f, nrow = N)
    }
    corrR <- corr_Y[(k + 1):p, (k + 1):p, drop = FALSE]
...
continuous_ss_sdf_fast_cpp <- function(f,
                                       R,
                                       sim_length,
                                       psi0 = 1,
                                       r = 0.001,
                                       ...
                                       intercept = TRUE,
                                       force_rebuild = FALSE) {
  if (!load_continuous_ss_sdf_fast_cpp(force_rebuild = force_rebuild)) {
    error_message <- continuous_ss_sdf_fast_cpp_error()
  ...
  cpp_inputs <- prepare_continuous_ss_sdf_fast_cpp_inputs(
    f = f,
    R = R,
    psi0 = psi0,
    r = r,
  ...
  cpp_impl <- continuous_ss_sdf_fast_cpp_function()
...
continuous_ss_sdf_fast <- function(f,
                                   R,
                                   sim_length,
                                   psi0 = 1,
                                   r = 0.001,
  ...
  result <- continuous_ss_sdf_fast_r(
    f = f,
    R = R,
    sim_length = sim_length,
    psi0 = psi0,
