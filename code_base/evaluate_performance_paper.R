...
evaluate_performance_paper <- function(
  ...
  ## -------------------------------------------------------------------------
  ## 11. Generate LaTeX Table (Table 6 Panel B)
  ## -------------------------------------------------------------------------

  if (verbose) cat("Generating Table 6 Panel B...\n")

  date_start <- format(min(oos_wide$date), "%Y:%m")
  date_end <- format(max(oos_wide$date), "%Y:%m")
  T_obs <- nrow(oos_wide)

  # Use short model list
  model_order <- c("BMA-20%", "BMA-40%", "BMA-60%", "BMA-80%",
                   "Top-80%", "Top-MPR-80%", "KNS", "RP-PCA",
                   "FF5", "HKM", "MKTB", "MKTS", "EW")
  models_avail <- intersect(model_order, perf_df_display$Model)
  df <- perf_df_display %>% filter(Model %in% models_avail)
  ...
  latex <- c(latex, "\\caption{Out-of-Sample Trading Performance}\\label{tab:oos-trading}\\vspace{-2mm}")
  ...
  latex <- c(latex, paste0(" \\multicolumn{", n_models + 1, "}{c}{\\textbf{Panel B: Out-of-sample} -- ",
  ...
  latex <- c(latex, paste0("Out-of-sample trading performance. ",
  ...
  table_file <- file.path(tables_dir, "table_6_panel_b_trading.tex")
  writeLines(latex_table, table_file)
  if (verbose) cat("Table 6 Panel B saved to: ", table_file, "\n")
