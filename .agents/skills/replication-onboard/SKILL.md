...
3. Use `tools/bootstrap_packages.R` or the platform wrapper to install missing packages. The script prints per-package progress with `[N/total]` format — read stdout directly for progress. Do NOT spawn monitor agents or background watchers. Wait for the script to complete. **If packages fail:** the script writes a diagnostic log to `tools/bootstrap_packages.log`. Read this log, diagnose the root cause (usually missing system `-dev` libraries), install the missing system deps via `bash tools/bootstrap_system.sh` or targeted `sudo apt install`, then re-run `bootstrap_packages.R`. Do NOT report failures to the user without first attempting to fix them.
